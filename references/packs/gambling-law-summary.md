# Gambling Law Summary Reference Pack

Detailed drafting rules for `skills/specialists/gambling-law-summary.md`. Load this pack only when drafting, tailoring, or quality-checking a gambling-law regulatory summary.

## Output Template

### Header

Include title, jurisdiction(s), effective date range, prepared date, and audience.

### Executive Summary

Use 5-12 bullets covering:

- Legal status by vertical.
- Primary regulator(s) and market entry barriers.
- High-risk compliance obligations and tax/fee exposure.
- Active enforcement priorities and pending reforms.

### Scope Table

| Item | Detail |
|---|---|
| Jurisdiction level | Federal / State / Tribal / Local |
| Legal authority | Constitution, statute, regulation, compact |
| Gaming verticals | Included and excluded |
| Regulator(s) | Agency names and jurisdiction |
| Sources | Statutes, regs, agency guidance, case law |

### Legal Status and Authority

- Identify permitted, prohibited, and restricted categories with statutory basis.
- Explain preemption hierarchy.
- Include tribal framework and compact status if applicable.

### Regulatory Bodies

- Identify primary and secondary regulators.
- Summarize licensing board composition and powers where relevant.

### Licensing and Suitability

| License Type | Who Needs It | Key Requirements | Term | Transferable? |
|---|---|---|---|---|
| Operator | | | | |
| Vendor | | | | |
| Key employee | | | | |
| Manufacturer | | | | |

Cover eligibility, background checks, financial suitability, ownership thresholds, fees, renewal cadence, and grounds for denial, suspension, or revocation.

### Operational Restrictions

Cover as applicable:

- Responsible gaming.
- AML/KYC.
- Age verification.
- Advertising limits.
- Game integrity testing.
- Technical standards.
- Audit requirements.
- Geolocation/server rules for online activity.
- Payment restrictions and betting limits.

### Taxes and Fees

| Category | Rate/Method | Trigger | Notes |
|---|---|---|---|
| GGR tax | | | |
| License fees | | | |
| Regulatory assessments | | | |
| Local taxes | | | |

### Enforcement and Penalties

- Administrative, civil, and criminal sanctions.
- Recent enforcement trends and exemplar actions.

### Pending Reforms

- Bill/rule ID, status, and expected effective date.
- Transition, grandfathering, and operational-impact rules.

### Risk and Compliance Recommendations

- Top 5 compliance priorities by vertical.
- Immediate action items and monitoring plan.

### Citation and Disclaimer

- Use Bluebook format where appropriate.
- Hyperlink official sources.
- Mark uncertain items `[Unverified]`.
- Include disclaimer: gambling law changes rapidly; summary is informational and not legal advice.

## Pitfalls

- Never blend federal, state, and tribal regimes; separate each authority clearly.
- Always flag IGRA, compacts, or federal preemption issues when relevant.
- Use only primary sources or regulator guidance for legal status assertions.
- Include effective dates and amendment history for all cited authorities.
- Label uncertainty explicitly and avoid speculative conclusions.
- Keep tone executive-ready and compliance-focused.
